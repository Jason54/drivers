...
class SNSPD():
    ...
    def get_idn(self):
        ...
        return self.visaobj.query('+A?')

    def get_channel_idn(self):
        return self.visaobj.query('+M?')
    def set_channel_idn(self, number):
        return self.visaobj.write('+M{};'.format(number))
    
    def get_bias_current(self):
        return float(50e-6*int(self.visaobj.query('+B?'))/1023)
    def set_bias_current(self, value):
        return self.visaobj.write('+B{};'.format(int(1023*value/50e-6)))
