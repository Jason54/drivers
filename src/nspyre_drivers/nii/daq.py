...
class DAQ:
    def __init__(self, serial_num=None):
        ...
        for d in self.system.devices:
            logger.debug(f'Found nidaqmx device [{d.name}] with serial number [{self.dec_sn_to_hex(d.serial_num)}]')
            if serial_num is None or d.serial_num == self.hex_sn_to_dec(serial_num):
                self.dev = d
        # check whether the device was found
        if self.dev is not None:
            logger.info(f'Bound to nidaqmx device [{self.dev.name}] with serial number [{self.dec_sn_to_hex(self.dev.serial_num)}]')
        else:
    ...
    def __repr__(self):
        if self.dev is not None:
            return f'DAQ(name={self.dev.name}, serial_num={self.dec_sn_to_hex(self.dev.serial_num)})'
        else:
            return 'DAQ(Not connected)'

    def serial_number(self):
        """Return the device serial number as a hex string"""
        return self.dec_sn_to_hex(self.dev.serial_num)
